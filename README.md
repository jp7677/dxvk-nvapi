...
- `DXVK_NVAPI_LATENCY_MARKER_LOG`, when set to `1`, prints only D3D, D3D12, and Vulkan latency marker events without enabling full `trace` logging. Each line contains the API, frame ID, marker type, marker value, and a QPC timestamp in microseconds.
...
```bash
meson setup --reconfigure --cross-file "./build-win64.txt" --buildtype "debugoptimized" -Denable_tests=True buildDir
