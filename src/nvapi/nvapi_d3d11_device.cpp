...
namespace dxvk {
    ...
    std::pair<Com<ID3D11VkExtDevice>, Com<ID3D11VkExtContext>> GetDxvkDevice(IUnknown* device) {
        Com<ID3D11Device> d3d11Device;
        Com<ID3D11DeviceContext> d3d11DeviceContext;

        if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&d3d11Device))))
            d3d11Device->GetImmediateContext(&d3d11DeviceContext);
        else if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&d3d11DeviceContext))))
    ...
    NvapiD3d11Device* NvapiD3d11Device::GetOrCreate(IUnknown* device) {
        std::scoped_lock lock{m_mutex};

        if (auto nvapiD3D11Device = Get(device))
            return nvapiD3D11Device;

        auto [dxvkDevice, dxvkContext] = GetDxvkDevice(device);
        ...
        auto [itI, inserted] = itF == m_nvapiDeviceMap.end()
            ? m_nvapiDeviceMap.emplace(device, std::make_shared<NvapiD3d11Device>(dxvkDevice.ptr(), dxvkContext.ptr()))
    ...
    NvapiD3d11Device* NvapiD3d11Device::Get(IUnknown* device) {
    ...
    NvapiD3d11Device::NvapiD3d11Device(ID3D11VkExtDevice* dxvkDevice, ID3D11VkExtContext* dxvkContext)
        : m_dxvkDevice(dynamic_cast<ID3D11VkExtDevice1*>(dxvkDevice)), m_dxvkContext(dynamic_cast<ID3D11VkExtContext1*>(dxvkContext)) {
        ...
        m_supportsExtMultiDrawIndirectCount = m_dxvkDevice->GetExtensionSupport(D3D11_VK_EXT_MULTI_DRAW_INDIRECT_COUNT);
