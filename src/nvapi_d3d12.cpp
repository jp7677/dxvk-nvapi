...
NVAPI_FUNCTION NvAPI_D3D12_GetRaytracingCaps(ID3D12Device* pDevice, NVAPI_D3D12_RAYTRACING_CAPS_TYPE type, void* pData, size_t dataSize) {
    ...
    switch (type) {
        ...
        case NVAPI_D3D12_RAYTRACING_CAPS_TYPE_CLUSTER_OPERATIONS:
            ...
            supported = env::isD3d12NvShaderExtnEnabled() && device && device->IsClusterAccelerationStructureSupported()
                ? NVAPI_D3D12_RAYTRACING_CLUSTER_OPERATIONS_CAP_STANDARD
                : NVAPI_D3D12_RAYTRACING_CLUSTER_OPERATIONS_CAP_NONE;
            *static_cast<NVAPI_D3D12_RAYTRACING_CLUSTER_OPERATIONS_CAPS*>(pData) = supported
                ? NVAPI_D3D12_RAYTRACING_CLUSTER_OPERATIONS_CAP_STANDARD
                : NVAPI_D3D12_RAYTRACING_CLUSTER_OPERATIONS_CAP_NONE;
            break;
        ...
        case NVAPI_D3D12_RAYTRACING_CAPS_TYPE_PARTITIONED_TLAS:
            ...
            supported = device && device->IsPartitionedAccelerationStructureSupported();
            *static_cast<NVAPI_D3D12_RAYTRACING_PARTITIONED_TLAS_CAPS*>(pData) = supported
