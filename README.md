...
The Vulkan Reflex layer, when built as a Linux-side layer, is not installed into Wine prefix. Instead it's installed like any other Vulkan implicit layer. It is distributed as a pair of files:
...
2. If needed, edit the layer manifest to ensure that its `library_path` points to the layer library
     ...
     - Finally, if the layer is in one of the paths searched by dynamic linker (like `/usr/lib`, this depends on the distro), it's possible to set the library path to just the name of the layer library: `"library_path": "libdxvk_nvapi_vkreflex_layer.so"`

Run `vulkaninfo` and check the list of discovered layers to verify there is an entry named `VK_LAYER_DXVK_NVAPI_reflex (DXVK-NVAPI Vulkan Reflex compatibility layer)`.
...
- `DXVK_NVAPI_VKREFLEX=1` enables the layer
